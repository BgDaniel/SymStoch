...
t
\section{Contravariance of the drift}
...
\mathrm{dY_{t}=\frac{d\phi}{dx}\left(\Xt\right)\left[\sig\left(\Xt\right)d\Wt+\mu\left(\Xt\right)dt\right]+\R\left(\phi, \sig\right)(\Xt)}
\end{equation} with 
\begin{equation}
...
\end{equation} is called the Laplacian operator on $\mathrm{\U}$ with respect to $\mathrm{\g}$.
\end{defi}
...
\begin{equation}
\mathrm{dZ^{\prime k}_{t}= \dots + \frac{\partial x^{\prime k}}{\partial x^{\ell}}\mu^{\ell}+g^{ij}\frac{\partial^2 x^{\prime k}}{\partial x^{i}\partial x^{j}}}
...
\begin{equation}
\mathrm{g^{ij}\frac{\partial^2 x^{\prime k}}{\partial x^{i}\partial x^{j}}= 0}
...
\end{equation} As 1) is equivalent to $\mathrm{\mathrm{g^{ij}\Gamma^{k}_{ij}}=0}$ for all $\mathrm{1 \leq k \leq n}$, it follows from condition 2) that
\begin{equation}
\mathrm{0 = g^{ij}\frac{\partial^{2}}{\partial x^{i}\partial x^{j}}}
...
\begin{lemm} Let $\mathrm{\phi: U\rightarrow U}$ be a local change of coordinates $\mathrm{x^{\prime k}=\phi^{k}\left(x\right)}$ where $\mathrm{x^{j}}$ and $\mathrm{\phi^{k}}$ are satisfying the conditions of lemma \cite{contravariance of drift}, then the transformation $\mathrm{Z^{\prime k}}$ under $\phi^{k}$ of any process solving 
\begin{equation}
\mathrm{dX^{i}_{t}= \sigma^{i}_{j}\left(\Z\right)dW^{j}_{t}}
\end{equation} is again a solution of the original equation, i.e.
\begin{equation}
\mathrm{dX^{\prime k}_{t}= \sigma^{k}_{\ell}\left(\Z^{\prime}\right)dW^{\ell}_{t}}
\end{equation} 
\end{lemm}
\begin{proof} As the starting process is driftless and the local coordintes as well as the change of local coordinates both meet the conditions of lemma, the transformed drift will have zero drift by the contravariance lemma. Hence,  the lemma is proven as soon as we can verify that the quadratic variation $d[\Z,\Z]$ of $\Z$  and the corresponding quadratic variation $d[\Ztil,\Ztil]$ of $\Ztil$ coincides. 

Since $\phi$ is assumed to be an isometry with respect to $g_{ij}$, we have $\mathrm{g^{\prime}_{k\ell}=g_{k\ell}}$ or equivalently
\begin{equation}
\mathrm{g_{rs}\frac{\partial x^{\prime r}}{\partial x^{k}}\frac{\partial x^{s}}{\partial x^{\ell}}=g_{k\ell}}
\end{equation} Using the definition of $\mathrm{g}$, we get 
\begin{equation}
\mathrm{\sigma^{i}_{r}\sigma^{i}_{s}\frac{\partial x^{\prime r}}{\partial x^{k}}\frac{\partial x^{\prime s}}{\partial x^{\ell}}=\sigma^{i}_{k}\sigma^{i}_{\ell}}
\end{equation}
which in turn by multiplying with $\mathrm{\frac{\partial x^{}}{}}$ and $\mathrm{}$

\end{proof}

\begin{lemm}\label{lemma0} Let $\phi\dbp\U\arr\U$ be an isometry with respect to the metric $\g$ given by the local matrix representation $\g= \left(\sigma\sigma^{\Tra}\right)^{-1}$ and let $\Z$ be a solution of 
...
\end{equation} As booth processes $\Z$ as well as $\Ztil$ are drifless, the lemma is proven Here $d[\Z,\Z]$ and so $d[\Ztil,\Ztil]$ are understood to be matrices given by 
\begin{equation}
